 ...
 */
function pickWorkspaceDatabase(autopick, hint) {
    const sqlite_file_extensions = ["db", "db3", "sqlite", "sqlite3", "sdb", "s3db"];
    const promise = new Promise((resolve) => {
        vscode_1.workspace.findFiles('**/*.{' + sqlite_file_extensions.join(",") + '}').then((filesUri) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        showAutoQuickPick(autopick, promise, hint).then(item => {
            if (item instanceof QuickPick.DatabaseItem) {
                ...
                vscode_1.window.showOpenDialog({ filters: { "Database": sqlite_file_extensions } }).then(fileUri => {
                    if (fileUri) {
