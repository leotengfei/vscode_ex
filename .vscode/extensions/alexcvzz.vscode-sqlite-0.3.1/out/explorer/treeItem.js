...
class DBItem extends SQLItem {
    constructor(dbPath, command) {
        super(path_1.basename(dbPath), vscode_1.TreeItemCollapsibleState.Collapsed, command);
        this.dbPath = dbPath;
        this.iconPath = {
...
class TableItem extends SQLItem {
    constructor(name, command) {
        super(name, vscode_1.TreeItemCollapsibleState.Collapsed, command);
...
class ColumnItem extends SQLItem {
    constructor(name, type, notnull, pk, defVal, command) {
        super(name + ` : ${type.toLowerCase()}`, vscode_1.TreeItemCollapsibleState.None, command);
        this.name = name;
        this.type = type;
